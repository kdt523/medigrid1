...
def get_alert_stats():
    try:
        ...
        resolved_today = query.filter_by(status='resolved').filter(
            Alert.resolved_at >= datetime.utcnow().replace(hour=0, minute=0, second=0)
        ).count()
...
def resolve_alert(alert_id):
    try:
        ...
        if user.role not in ['hospital_admin', 'system_admin', 'authority']:
